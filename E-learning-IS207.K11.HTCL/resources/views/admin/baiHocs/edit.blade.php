        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.baiHoc.title_singular') }}
                </div>
                <div class="panel-body">

                    <form method="POST" action="{{ route("admin.bai-hocs.update", [$baiHoc->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('lop') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="lop_id" id="lop_id" required>
                                @foreach($lops as $id => $lop)
                                    <option value="{{ $id }}" {{ ($baiHoc->lop ? $baiHoc->lop->id : old('lop_id')) == $id ? 'selected' : '' }}>{{ $lop }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('ten_bai_hoc') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="ten_bai_hoc" id="ten_bai_hoc"
                                oninput="join_names();" onpaste="join_names();"
                                value="{{ old('ten_bai_hoc', $baiHoc->ten_bai_hoc) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('lien_quan') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="lien_quan" id="lien_quan"
                                value="{{ old('lien_quan', $baiHoc->lien_quan) }}" readonly="readonly">
                        ...
                        <div class="form-group {{ $errors->has('loi_ngan') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="loi_ngan" id="loi_ngan" value="{{ old('loi_ngan', $baiHoc->loi_ngan) }}">
                        ...
                        <div class="form-group {{ $errors->has('noi_dung') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="noi_dung" id="noi_dung">{{ old('loi_ngan', $baiHoc->loi_ngan) }}</textarea>
