        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.theLoai.title_singular') }}
                </div>
                <div class="panel-body">

                    <form method="POST" action="{{ route("admin.the-loais.update", [$theLoai->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('ten_tl') ? 'has-error' : '' }}">
                            <label class="required" for="ten_tl">{{ trans('cruds.theLoai.fields.ten_tl') }}</label>
                            <input class="form-control" type="text" name="ten_tl" id="ten_tl" value="{{ old('ten_tl', $theLoai->ten_tl) }}" required>
